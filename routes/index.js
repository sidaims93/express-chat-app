module.exports = (database, redis) => {
  ...
  router.get('/', checkAuth, async function(req, res) {
    ...
    const redisKey = `Dashboard:Payload:${userId}`;
    var redisKeyExists = await redis.exists(redisKey);
    //console.log(redisKeyExists);
    var payload;
    if(redisKeyExists) {

      console.log('Found in redis so getting from it');
      payload = await redis.get(redisKey);
      payload = JSON.parse(payload);
      //await redis.del(redisKey); //Uncomment this when u want the redis key to expire.
  ...
  router.post('/post/message', checkAuth, async(req, res) => {
    ...
    var redisKey = `POST:MESSAGE:${user_id}`;
    var redisKeyExists = await redis.exists(redisKey);

    if(redisKeyExists) {
      res.json({"status": false, "message": "Too many requests"}).status(429);
      return false;
    }
